...
def model(weights='imagenet',
    ...
    # Concatenation
    block1 = MaxPooling2D((4, 4), strides=(4, 4))(block1)
    block2 = MaxPooling2D((2, 2), strides=(2, 2))(block2)
    block3 = block3
    block4 = UpSampling2D(size=2)(block4)
    block5 = UpSampling2D(size=4)(block5)
    # concat = Concatenate([block1, block2, block3, block4, block5])
    concat = Lambda(lambda x: tf.concat(x, -1) )([block1, block2, block3, block4, block5])
    out = Conv2D(12, (3, 3), activation='relu', padding='same')(concat)    
